...
Apply migrations once, in filename order, from the API directory:

```bash
cd /var/www/mainzware/api
for file in migrations/*.sql; do echo "Applying $file"; sudo -u postgres psql -d mainzworld -f "$file" || exit 1; done
```
